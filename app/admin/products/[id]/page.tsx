...
export default function EditProductPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const response = await productService.getProduct(productId); // View count handled by IP tracking
      const productData = response.data;
      setProduct(productData);
      
      // Convert specifications array to string for editing
      let specsText = "";
      if (productData.specifications && Array.isArray(productData.specifications)) {
        specsText = productData.specifications
      ...
      if (productData.images && productData.images.length > 0) {
        ...
        setImagePreviews(productData.images.map((img: any) => img.url || img));
      }
    } catch (error: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFiles.length > 0) {
        ...
        newUploadedImages = uploadResult.data.map((img: any) => ({
          url: img.url,
    ...
    return (
      <div className="text-center py-12">
        ...
        <button
          ...
          className="mt-4 text-[#39b54a] hover:underline"
        >
          Quay lại danh sách
        </button>
      </div>
