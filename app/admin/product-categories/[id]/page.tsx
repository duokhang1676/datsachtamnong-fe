...
export default function EditProductCategoryPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("Đã cập nhật danh mục sản phẩm thành công!");
      router.push("/admin/product-categories");
    } catch (error: any) {
      console.error("Error updating category:", error);
      alert(error.response?.data?.message || "Không thể cập nhật danh mục. Vui lòng thử lại!");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div>
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Chỉnh sửa danh mục sản phẩm</h1>
